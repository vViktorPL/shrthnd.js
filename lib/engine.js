var css = require('css');
var allShorthands = require('./properties');

module.exports = function(cssString, options) {
  var cssObject = css.parse(cssString);
  var longPropertiesPositions = [];
  var shorthands = allShorthands;

  options = options || {};

  if ('ignoreProperty' in options) {
    var ignoreProperties = options.ignoreProperty;
    if (!Array.isArray(ignoreProperties)) {
      ignoreProperties = [ignoreProperties];
    }

    shorthands = allShorthands.filter(
      function(shorthand) {
        return ignoreProperties.indexOf(shorthand.shorthandProperty) === -1;
      }
    );
  }
